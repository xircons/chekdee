...
func InitializeServer(logger *slog.Logger) (*server.Server, error) {
	...
	leaveAttachmentRepository := repository.NewLeaveAttachmentRepository(pool)
	auditLogRepository := repository.NewAuditLogRepository(pool)
	auditLogUsecase := usecase.NewAuditLogUsecase(auditLogRepository)
	leaveUsecase := usecase.NewLeaveUsecase(leaveRequestRepository, leaveAttachmentRepository, auditLogUsecase, riverInsertClient)
